...
class MultiVector:
    ...
    # FINAL, CORRECTED fromkeysvalues in MultiVector class
    @classmethod
    def fromkeysvalues(cls, algebra: Any, keys: Tuple[int, ...], values: Union[List[Any], np.ndarray]) -> 'MultiVector':
        """
        Primary constructor. Robustly handles shape, NumPy ambiguity, and empty/zero cases.
        """
        if not isinstance(keys, tuple):
            keys = tuple(keys)

        # --- NumPy-aware check for zero values ---
        is_zero = False
        if isinstance(values, np.ndarray):
            # Use np.all for NumPy arrays
            is_zero = not values.any()
        else:
            # Use standard `all` for lists and other iterables
            try:
                is_zero = all(v == 0 for v in values)
            except TypeError: # Catches cases where values might not be iterable, e.g., a single int
                is_zero = (values == 0)

        # Handle truly empty multivector (no keys, no values)
        ...
        # --- NumPy-aware check for empty values ---
        values_exist = True
        if isinstance(values, np.ndarray):
            values_exist = values.size > 0
        else:
            # This handles lists, tuples, and also catches single non-iterable values
            # `if not [0]` is False, `if not []` is True
            if not values and values != 0:
                values_exist = False

        if not values_exist and keys:
            values = [0] * len(keys)

        # --- Object Creation and Shape Determination ---
        obj = object.__new__(cls)
        obj.algebra = algebra
        obj._keys = keys
        obj._values = values
        ...
        if isinstance(values, np.ndarray):
            ...
            elif values.ndim > 1 and values.shape[0] == len(keys):
                obj.shape = values.shape[1:]
        elif isinstance(values, list) and values and isinstance(values[0], (list, np.ndarray)):
            try:
                arr = np.array(values)
                if len(keys) == 1: obj.shape = arr.shape
                elif arr.shape[0] == len(keys): obj.shape = arr.shape[1:]
    ...
    def is_effectively_zero(self, tolerance: float = 1e-12) -> bool:
        """
        Check if this multivector is effectively zero, handling symbolic expressions.
        
        Args:
            tolerance: Numerical tolerance for zero detection
            
        Returns:
            True if the multivector is effectively zero
        """
        if not hasattr(self, '_keys') or not self._keys:
            return True
        ...
        for v in self._values:
            ...
            if isinstance(v, (int, float, complex, np.number)):
                ...
                if np.any(np.abs(v) >= tolerance):
                    return False
            elif hasattr(v, 'is_zero'):
                # SymPy expressions have is_zero property
                try:
                    if not v.is_zero:
                        return False
                except (AttributeError, TypeError):
                    # If is_zero fails, try numerical evaluation
                    try:
                        if abs(complex(v)) >= tolerance:
                            return False
                    except (ValueError, TypeError):
                        # If we can't evaluate, assume non-zero
                        return False
            else:
                ...
                try:
                    ...
                    # If we can't evaluate, check if it's string '0'
                    if str(v) != '0':
                        return False
    ...
    # === Arithmetic Operations ===
        ...
        # In multivector.py
    # REPLACE the __eq__ method with this one.

    def __eq__(self, other: Any) -> bool:
        """Definitive equality check."""
        # Step 1: Promote `other` to a MultiVector if it's a scalar.
        # This simplifies all subsequent logic to a single MV-MV comparison.
        if isinstance(other, (int, float, complex, np.number, sympy.Expr)):
            try:
                other = self.algebra.scalar(other)
            except Exception:
                return NotImplemented

        # Step 2: Now, if `other` is not a MultiVector, they can't be equal.
        if not isinstance(other, MultiVector):
            return NotImplemented

        # Step 3: Perform a robust MV-MV comparison.
        if self.algebra != other.algebra: return False
        if self.shape != other.shape: return False

        # A fast path for comparing zero multivectors
        self_is_zero = self.keys() == (0,) and self.values() == [0]
        other_is_zero = other.keys() == (0,) and other.values() == [0]
        if self_is_zero or other_is_zero:
            return self_is_zero and other_is_zero

        # Full comparison of all components
        all_keys = set(self.keys()) | set(other.keys())
        for k in all_keys:
            v_self = self.get(k, 0)
            v_other = other.get(k, 0)

            # Use a single, robust comparison helper
            if not self._are_coeffs_equal(v_self, v_other):
                return False
                
        return True

    def _are_coeffs_equal(self, c1: Any, c2: Any) -> bool:
        """Robustly compare two coefficients of any potential type."""
        if isinstance(c1, sympy.Expr) or isinstance(c2, sympy.Expr):
            return sympy.simplify(sympy.S(c1) - sympy.S(c2)) == 0
        if isinstance(c1, np.ndarray) or isinstance(c2, np.ndarray):
            return np.allclose(np.asarray(c1), np.asarray(c2), atol=1e-9)
        try:
            return abs(c1 - c2) < 1e-9
        except TypeError:
            return c1 == c2 # Fallback for non-numeric types
    ...
    def rcontract(self, other):
        """Right contraction."""
        return self.algebra.rc(self, other)
    
    def lc(self, other):
        """Left contraction (short form)."""
        return self.algebra.lc(self, other)
    
    def rc(self, other):
        """Right contraction (short form)."""
        return self.algebra.rc(self, other)
    
    def is_bivector(self):
    # Bivector = only grade-2 elements present
        return self.grades == (2,)

    def is_zero(self):
        """Returns True if this MV is (numerically) zero."""
        # You may want a more robust zero check (tol, all close to zero, etc.)
        return all(float(v) == 0.0 for v in self._values)
    
    def __lshift__(self, other):
        """Left contraction operator (<<)."""
        return self.lcontract(other)
    ...
    def __getitem__(self, item: Union[int, Tuple, slice]) -> Any:
        ...
        - Integer `k`: Returns the *value for blade key k* (not its position).
        ...
        """
        # Blade-key lookup (for int)
        if isinstance(item, int):
            # Blade-aware: search for key==item
            if hasattr(self, '_keys') and hasattr(self, '_values'):
                if item in self._keys:
                    idx = self._keys.index(item)
                    return self._values[idx]
                else:
                    raise IndexError(f"Blade key {item} not present in this MultiVector")
            else:
                raise IndexError("MV not initialized")
        # Slices of all keys/values
            ...
            if hasattr(self, '_keys') and hasattr(self, '_values'):
                ...
                raise IndexError("MV not initialized")
        elif isinstance(item, tuple): # For array-valued MVs
            ...
            raise TypeError(f"MV indices must be integers, slices, or tuples, not {type(item).__name__}")
